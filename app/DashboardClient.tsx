...
import { motion } from 'framer-motion'; // <-- Impor untuk animasi
import Papa from 'papaparse'; // <-- Impor untuk ekspor CSV
...
// Tipe Data, Helper Function parseDate, dan Komponen StatCard (tidak berubah)
...
const parseDate = (dateString: string): Date | null => { if (!dateString) return null; const parts = dateString.split('-'); if (parts.length !== 3) return null; const day = parseInt(parts[0], 10); const year = parseInt(parts[2], 10); const monthMap: { [key: string]: number } = { 'Jan': 0, 'Feb': 1, 'Mar': 2, 'Apr': 3, 'Mei': 4, 'Jun': 5, 'Jul': 6, 'Agu': 7, 'Sep': 8, 'Okt': 9, 'Nov': 10, 'Des': 11 }; const month = monthMap[parts[1]]; if (isNaN(day) || isNaN(year) || month === undefined) return null; return new Date(year, month, day, 12); };
const StatCard = ({ title, value, className = '' }: { title: string; value: string | number, className?: string }) => ( <div className={`p-6 rounded-lg shadow-lg text-white flex flex-col justify-center items-center ${className}`}> <h3 className="text-sm uppercase text-gray-300 font-bold tracking-wider">{title}</h3> <p className="text-4xl font-extrabold mt-2">{value}</p> </div> );
...
export default function DashboardClient({ initialData }: DashboardClientProps) {
  ...
  // STATE BARU: Untuk paginasi tabel
  const [currentPage, setCurrentPage] = useState(1);
  const itemsPerPage = 20; // Tampilkan 20 data per halaman

  // Logika filter bertingkat (tidak berubah)
  ...
  useEffect(() => { setCurrentPage(1); }, [filteredData]); // Jika data berubah, kembali ke halaman 1

  // LOGIKA BARU: Data untuk ditampilkan di tabel (sudah dipaginasi)
  ...
  // HANDLER BARU: Untuk ekspor data ke CSV
  const handleExportCSV = () => {
    const csv = Papa.unparse(filteredData); // Gunakan `filteredData` (semua hasil filter), bukan `paginatedData`
  ...
  return (
    <div>
      <motion.div {...animationProps}>
        {/* ... (Kartu Statistik & Filter tidak berubah) ... */}
      ...
      <motion.div {...animationProps} transition={{ duration: 0.5, delay: 0.3 }}>
        <div className="overflow-x-auto bg-white dark:bg-gray-800 shadow-lg rounded-lg p-4">
            {/* UI BARU: Tombol Ekspor dan Info Data */}
            ...
            <table className="w-full text-sm text-left text-gray-600 dark:text-gray-400">
                ...
                <tbody>
                    {paginatedData.map((row: DataRow, index: number) => ( // <-- Gunakan `paginatedData`
