    ...
    <div className="w-full">
      ...
      {pemasukanStats.show && (
        <motion.div {...animationProps} transition={{ delay: 0.1 }} className="mb-12">
          <h2 className="text-2xl font-bold mb-6 text-gray-800 dark:text-white px-2 tracking-tight">Analisis Pemasukan Bulanan</h2>
      ...
      <motion.div className="bg-white dark:bg-gray-800 shadow-lg rounded-lg p-6 mb-8" {...animationProps} transition={{ delay: 0.4 }}>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left">
                <thead className="text-xs text-gray-500 uppercase bg-gray-50 dark:bg-gray-700 tracking-wider">
                    <tr>{Object.keys(initialData[0] || {}).map((h, i) => <th key={i} className="px-4 py-4 whitespace-nowrap">{h}</th>)}</tr>
                </thead>
                    ...
                    {filteredData.slice((currentPage-1)*itemsPerPage, currentPage*itemsPerPage).map((row: any, i) => (
                      <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                          {Object.keys(initialData[0] || {}).map((h, j) => (
                            <td key={j} className="px-4 py-4 whitespace-nowrap dark:text-gray-300 font-medium">
                              {h === 'Link Video' && row[h] ? (
                                <div className="flex gap-2">
                                  <a href={row[h]} target="_blank" className="px-3 py-1.5 bg-indigo-600 text-white rounded-md text-xs font-bold hover:bg-indigo-700 transition-all shadow-sm">Lihat</a>
                                  <button onClick={() => handleCopyLink(row[h])} className={`px-3 py-1.5 rounded-md text-xs font-bold transition-all shadow-sm ${copiedLink === row[h] ? 'bg-green-500 text-white' : 'bg-gray-200 dark:bg-gray-600 text-gray-700 dark:text-gray-200 hover:bg-gray-300 dark:hover:bg-gray-500'}`}>
                                    {copiedLink === row[h] ? 'Tersalin' : 'Salin'}
                                  </button>
                                </div>
                              ) : (row[h] || '-')}
                            </td>
          ...
          <div className="flex justify-center mt-8 gap-4 items-center">
            <button disabled={currentPage === 1} onClick={() => setCurrentPage(p => p - 1)} className="px-4 py-2 bg-gray-200 dark:bg-gray-700 rounded-lg disabled:opacity-30 dark:text-white font-bold hover:bg-gray-300 dark:hover:bg-gray-600">Prev</button>
            <span className="text-xs font-bold text-gray-500 uppercase tracking-widest">Halaman {currentPage}</span>
